...
async function getTimeForZone(zone: string): Promise<{ time: string; isLive: boolean }> {
  try {
    const response = await fetch(`https://worldtimeapi.org/api/timezone/${zone}`, { next: { revalidate: 60 } })
...
export async function GET(request: Request) {
  ...
  if (zones.length === 0) {
    return NextResponse.json({ error: "No time zones provided" }, { status: 400 })
  }
